...
# This node should read from plant_pose json file,
# subscirbe to current position
# and publish the closest plant pose as goal_pose


class plantPose(Node):
    # publish rate
    RATE = 100

    def __init__(self):
        self.goal_pose
        plant_pose_file = "src/planning/plant_poses/test_plant_poses.json"
        with open(plant_pose_file, 'r') as checkpoints_file:
            self.CHECKPOINTS = (json.load(checkpoints_file))["checkpoints"]

        # publish trajectory to "traj/local_frame_traj"
        self.traj_publisher = self.create_publisher(Pose, "traj/goal_pose", 1)

        # ROS2 timer for stepping
        self.timer = self.create_timer(1.0 / self.RATE, self.step)

        super().__init__('goal_pose_finder')
...
def main(args=None):
    ...
    plant_pose = plantPose()
    rclpy.spin(plant_pose)
    rclpy.shutdown()
